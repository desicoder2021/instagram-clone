...
export const addNewPost = async (req, res) => {
  try {
    ...
    // Populate the post author field (excluding password)
...
export const likePost = async (req, res) => {
  try {
    ...
    if (postOwnerId !== likerId) {
      // emit notification only if liker and post owner are different
      const notification = {
        type: "like",
        userId: likerId,
        userDetails: user,
    ...
    return res.status(500).json({
      message: "An Unexpected error occured while liking the post",
      error: error.message,
      success: false,
    });
  }
...
export const dislikePost = async (req, res) => {
  try {
    ...
    return res.status(500).json({
      message: "An Unexpected error occured while disliking the post",
      error: error.message,
      success: false,
    });
  }
...
export const getCommmentsOfPost = async (req, res) => {
  try {
    const postId = req.params.id;
    // Check if post exists
    const post = await Post.findById(postId);
    ...
    console.error("Error fetching comments:0", error);
    return res.status(500).json({
      message: "An Unexpected error occured while fetching comments",
      error: error.message,
      success: false,
    });
  }
...
export const bookmarkPost = async (req, res) => {
  try {
    ...
    if (user.bookmarks.includes(post._id)) {
      // Post is aready bookmarked -> remove from the bookmarks
